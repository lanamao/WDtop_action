...
$(function () {
	...
	//動態	
	function start() {
		$('#we5_Logo').delay(1200).animate({ left: we5_Logo_left, top: we5_Logo_top, opacity: 1 },800);
		//$('#name').delay(1200).animate({ left: name_left, top: name_top, opacity: 1 },500);			
		$('#mainMenu').delay(1000).animate({ left: Menu_left, top: Menu_top, opacity: 1 }, 1000);
		$('#text01').delay(1200).animate({ left: Text01_left, top: Text01_top, opacity: 1 }, 800);
		$('#directions').delay(1200).animate({ left: Text02_left, top: Text02_top, opacity: 1 }, 800);
	}
	...
	$(document).ready(function () {
		$('#list').click(function () {
			$.fancybox({
				...
				'overlayOpacity': 0.5 ,
				'type': 'iframe',
				'href': '/Action/10_We5/20161229L/list.html'
			});
			return false;
		});
	});
